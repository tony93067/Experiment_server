...
void *handle_client(void *arg)
{
    ...
    */
    ...
    if (0 != getaddrinfo(SERVER_IP, "5200", &hints, &res))
    {
        cout << "illegal port number or port is busy.\n" << endl;
        exit(1);
    }
       ...
       if(ssize < sb.st_size)
        {
            ...
            {
                cout << "send UDT:" << UDT::getlasterror().getErrorMessage() << endl;
                exit(1);
            }
            else
            {
...
#endif
{
    ...
    if(mode == 1)
    {
        strcat(result_addr, "Sender_UDT2_Monitor_");
    ...
    {
        strcat(result_addr, "Sender_CTCP2_Monitor_");
        strcat(result_addr, "MSS");
        strcat(result_addr, temp_MSS);
        strcat(result_addr, "_TCP");
        strcat(result_addr, temp_BK_TCP);
    ...
    {
        strcat(result_addr, "Sender_CBiCTCP2_Monitor_");
        strcat(result_addr, "MSS");
        strcat(result_addr, temp_MSS);
        strcat(result_addr, "_TCP");
        strcat(result_addr, temp_BK_TCP);
    ...
    fout << "Time," <<"SendPacket," <<"SendRate(Mb/s)," << "FlightSize," << "ReceiveRate(Mb/s)," << "Send Loss," << "Recv Loss," << "RTT(ms)," << "CWnd," << "FlowWindow," << "Retrans," << "PktSndPeriod(us)," << "RecvACK," << "RecvNAK," << "EstimatedBandwidth(Mb/s)," << "Retrans_Total," << "NAK_TotalRecv," << "Total Send Loss," << "Total Recv Loss," << "Timeout," << "ExpCount," << "usSndDuration"<< endl;
    ...
    while (true)
    {
        ...
        fout << monitor_time << "," << perf.pktSent << "," << perf.mbpsSendRate << "," << perf.pktFlightSize << "," << perf.mbpsRecvRate << "," << perf.pktSndLoss << "," << perf.pktRcvLoss << ","<< perf.msRTT << "," << perf.pktCongestionWindow << ","
